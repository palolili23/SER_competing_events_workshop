...
- First, we run a set of conditional pooled logistic models, both as a first step to calculate the risk of prostate cancer death, and to calculate weights for death due to other causes, and weights for censoring
...
longProstRed %>% 
  mutate(cumPredC = cumPredC,
         ...
         weights_te_stab = weights_te_stab) %>% 
    filter(patno == 8) %>% slice(45:60) %>% 
...
longProstRed %>% 
  mutate(cumPredC = cumPredC,
         weights_te = weights_te,
         weights_te_stab = weights_te_stab) %>% 
ggplot(aes(as_factor(dtime), weights_te)) +
  geom_boxplot() +
...
longProstRed %>% 
  mutate(cumPredC = cumPredC,
         weights_te = weights_te,
         weights_te_stab = weights_te_stab) %>% 
ggplot(aes(as_factor(dtime), weights_te_stab)) +
  geom_boxplot() +
  labs(title = "Unstabilized weights for censoring")
```
...
longProstRed %>% 
  mutate(cumPredO = cumPredO,
         ...
         weights_de = weights_de,
         weights_de_stab = weights_de_stab) %>% 
    filter(patno == 487) %>% 
...
longProstRed %>% 
  mutate(cumPredO = cumPredO,
         ...
         weights_de = weights_de,
         weights_de_stab = weights_de_stab) %>% 
ggplot(aes(as_factor(dtime), weights_de)) +
  geom_boxplot() +
...
longProstRed %>% 
  mutate(cumPredO = cumPredO,
         ...
         weights_de = weights_de,
         weights_de_stab = weights_de_stab) %>% 
ggplot(aes(as_factor(dtime), weights_de_stab)) +
  geom_boxplot() +
